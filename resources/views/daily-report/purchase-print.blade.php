...
<head>
	...
	<title>{{ $companyDetails->company_name ?? 'R & A Veg Ltd' }} - Daily Purchase Report</title>
	<style>
		* { box-sizing: border-box; margin: 0; padding: 0; }
		...
		.contact-bar {
			background: #f3f4f6; padding: 20px 40px;
		...
		.data-table thead th {
			font-size: 13px; font-weight: 700; color: #f97316; text-align: left;
			padding: 12px 14px; border-bottom: 1.5px solid #fde6d3; white-space: nowrap;
		}
		.data-table thead th.num { width: 50px; }
		.data-table thead th.right { text-align: right; }
		.data-table thead th.center { text-align: center; }
		.data-table tbody td {
			padding: 14px; font-size: 13.5px; color: #0f1115;
		...
		.data-table .row-supplier { color: #0f1115; }
		.data-table .row-amount { font-weight: 700; }
		.data-table .right { text-align: right; }
		.data-table .center { text-align: center; }

		.empty-state {
			padding: 18px; text-align: center; color: #94a3b8;
			font-size: 13px; font-style: italic;
			background: #f9fafb; border-radius: 10px; border: 1px dashed #e5e7eb;
		}

		.data-table .total-row td {
			border-top: 2px solid #fde6d3; border-bottom: none;
			padding: 18px 14px; font-weight: 800; color: #f97316; font-size: 14px;
		}
		.data-table .total-row .total-label {
			color: #f97316; font-size: 14px; font-weight: 800;
		...
		.badge {
			...
			font-weight: 700; letter-spacing: 0.2px; border: 1px solid;
		}
		...
		.notes-section {
			margin: 24px 40px 32px; padding: 20px 24px;
			background: #fffaf5; border: 1px solid #fde6d3;
			border-left: 4px solid #f97316; border-radius: 12px;
		}
		.notes-section .notes-label {
			color: #f97316; font-weight: 700; font-size: 11px;
			text-transform: uppercase; letter-spacing: 1.2px; margin-bottom: 8px;
		}
		.notes-section .notes-text {
			color: #4b5563; font-size: 13px; line-height: 1.6; font-style: italic;
		}
	...
	<div class="page">
		...
		<div class="contact-bar">
			...
			<div class="contact-right">
				<div class="report-title">Daily Purchase Report</div>
		...
		<div class="notes-section">
			...
			<div class="notes-text">This statement reflects the purchase transactions as of the date shown. Please review and contact us for any discrepancies.</div>
		</div>
	</div>
</body>
</html>
