...
<head>
	...
	<title>{{ $companyDetails->company_name ?? 'R & A Veg Ltd' }} - Unassigned Suppliers</title>
	<style>
	...
	<div class="page">
		...
		<div class="contact-bar">
			...
			<div class="contact-right">
				<div class="report-title">Unassigned Suppliers</div>
				<div class="report-sub">Customer invoice items without an assigned supplier</div>
			</div>
		</div>
		...
		<div id="content" style="display:none">
			...
			<div class="notes-section">
				...
				<div class="notes-text" id="footer-text">Items below need supplier assignment to complete stock allocation.</div>
			</div>
		</div>
	</div>
...
(async function(){
	...
	document.title = 'Unassigned Suppliers — ' + periodText;

	try {
		const res = await fetch('{{ route("stock_closing.view.unassigned-suppliers.list") }}', {
			method:'POST',
			...
			body: JSON.stringify({search, from_date: fromDate, to_date: toDate})
		});
		...
		items.forEach(function(r, i){
			tbody += '<tr>'+
				...
				'<td>'+(r.invoice_id||'')+'</td>'+
				'<td>'+(r.created_at||'')+'</td>'+
				'<td class="row-product">'+(r.product_name||'')+'</td>'+
				'<td>'+(r.customer_name||'')+'</td>'+
				'<td class="right">'+(r.quantity||'')+'</td>'+
				'<td class="right">'+currency+' '+Number(r.unit_price||0).toFixed(2)+'</td>'+
				'<td>'+(r.remarks||'')+'</td>'+
			'</tr>';
		});
