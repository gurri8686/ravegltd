...
$labels = [
	'customer' => ['title' => 'Customer Return', 'party' => 'Customer', 'route' => route('customer_return.view.returns')],
	'supplier' => ['title' => 'Supplier Return', 'party' => 'Supplier', 'route' => route('supplier_return.view.returns')],
	'dump'     => ['title' => 'Dump',            'party' => 'Supplier', 'route' => route('dump_return.view.returns')],
];
	...
	<div class="page">
		...
		<div class="contact-bar">
			...
			<div class="contact-right">
				...
				<div class="report-sub">List of returns for the selected period</div>
			</div>
		</div>
		...
		<div id="content" style="display:none">
			...
			<div class="notes-section">
				...
				<div class="notes-text" id="footer-text">Records reflect data as of generation time.</div>
			</div>
		</div>
	</div>
...
(async function(){
	...
	const partyKey = '{{ $cfg['party'] === 'Customer' ? 'customer' : 'supplier' }}';
	...
	const body = isCustomer
		...
		: { supplier_id: entityId, date: fromDate || '2000-01-01', end_date: toDate };
	...
	try {
		...
		items = items.slice().sort(function(a,b){ return new Date((b.date||'').replace(' ','T')) - new Date((a.date||'').replace(' ','T')); });

		let tbody = '';
		items.forEach(function(r, i){
			const dateStr = r.date ? new Date((r.date||'').replace(' ','T')).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'}) : '';
			tbody += '<tr>'+
				'<td class="row-num">'+(i+1)+'</td>'+
				'<td>'+dateStr+'</td>'+
				'<td>'+(r.invoice_id||'')+'</td>'+
				'<td class="row-product">'+(r.product_id||'')+'</td>'+
				'<td>'+(r[partyKey]||'')+'</td>'+
				'<td>'+(r.note||'')+'</td>'+
				'<td class="right">'+(r.quantity||'')+'</td>'+
				'<td class="right">'+(r.price ? currency+' '+Number(r.price).toFixed(2) : '')+'</td>'+
				'<td class="right">'+(r.total ? currency+' '+Number(r.total).toFixed(2) : '')+'</td>'+
			'</tr>';
		});
		document.getElementById('table-body').innerHTML = tbody || '<tr><td colspan="9" style="text-align:center;padding:30px;color:#94a3b8">No records found</td></tr>';
		document.getElementById('footer-text').textContent = 'Generated on ' + new Date().toLocaleString('en-GB',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}) + ' · ' + items.length + ' record' + (items.length === 1 ? '' : 's') + ' shown.';
