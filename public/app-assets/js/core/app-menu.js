...
(function (window, document, $) {
  ...
  $.app.menu = {
    ...
    manualScroller: {
      ...
      init: function () {
        ...
        if (!$.app.menu.is_touch_device() && window.innerWidth <= 1024) {
          ...
          // Desktop (>1024px): scroll the navigation list directly
          if (window.innerWidth > 1024 && !window._desktopSidebarScrollReady) {
            window._desktopSidebarScrollReady = true;
            setTimeout(function() {
              var menuEl   = document.querySelector('.main-menu.menu-fixed');
              var navEl    = document.querySelector('.main-menu.menu-fixed .navigation-main');
              if (!menuEl || !navEl) return;

              var navbar    = document.querySelector('.header-navbar');
              var brandEl   = menuEl.querySelector('.sidebar-brand-logo');
              var logoutEl  = menuEl.querySelector('.sidebar-logout');
              var navH      = navbar   ? navbar.getBoundingClientRect().height   : 64;
              var brandH    = brandEl  ? brandEl.getBoundingClientRect().height  : 0;
              var logoutH   = logoutEl ? logoutEl.getBoundingClientRect().height : 0;
              var available = window.innerHeight - navH - brandH - logoutH - 10;

              navEl.style.setProperty('max-height', available + 'px', 'important');
              navEl.style.setProperty('overflow-y', 'auto',           'important');
              navEl.style.setProperty('overflow-x', 'hidden',         'important');
      ...
      updateHeight: function () {
        if (window.innerWidth > 1024) return; // desktop uses native scroll with fixed inline height
  ...
  $.app.nav = {
    ...
    adjustSubmenu: function ($menuItem) {
      ...
      var navbarH = $('.header-navbar').outerHeight() || 60;
      var rect = $menuItem[0].getBoundingClientRect();
      var topPos = Math.round(rect.top) - Math.round(rect.height);

      // Add title inside popup
      var title = $menuItem.find('> a span.menu-title').text().trim();
      if (title) {
        ul.prepend('<li class="flyout-title" style="padding:12px 20px 8px;font-weight:700;font-size:11px;letter-spacing:1px;color:#F27420;text-transform:uppercase;border-bottom:2px solid #F27420;list-style:none;">' + title + '</li>');
      }

      ul.addClass('menu-popout').appendTo('.main-menu-content');

      // Measure then position
      var popH = ul.outerHeight();
      var maxH = winH - navbarH - 20;

      // If popup doesn't fit below, shift up
      if (topPos + popH > winH - 10) {
        topPos = winH - popH - 10;
      }
      // Don't go above navbar
      if (topPos < navbarH) topPos = navbarH;

      // Calculate available height from topPos to bottom
      var availH = winH - topPos - 10;
      if (availH > maxH) availH = maxH;

      ul[0].style.cssText += 'position:fixed !important;top:' + topPos + 'px !important;max-height:' + availH + 'px !important;overflow-y:auto !important;overflow-x:hidden !important;';
    },
