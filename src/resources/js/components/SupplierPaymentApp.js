...
const {setSuppliers, setCurrentSupplier, setSuppliersLoading, triggerPaymentRefresh,
	setPaymentMode, setAmount, setDate, setNote, setInvoices, resetSuppliersState,setTotalChecked,
	setPaymentDoable, toggleInvoice, resetAccountPayments,resetPaymentForm, resetInvoices, setAccountPayments, setOnAccount, setSummaryTotals, triggerSave} = suppliersSlice.actions;
...
// form .
function CreateForm({ apiUrl, onSubmit, initialSupplierId, currency }) {
	const dispatch = useDispatch();
			...
			try {
				...
				if (res.data.success) {
					...
					if (initialSupplierId) {
						...
						if (s) dispatch(setCurrentSupplier(s.id));
					} else if (res.data.payload.length > 0) {
						dispatch(setCurrentSupplier(res.data.payload[0].id));
					}
				}
	...
	const inputFocus = {borderColor:'rgb(234, 88, 12)',background:'#fff',boxShadow:'0 0 0 4px rgba(234,88,12,0.08)'};
	...
	const { summaryTotals } = useSelector(state => state.suppliers);
	const cur = currency || '£';
	const fmtS = v => cur + ' ' + Number(v||0).toLocaleString('en-GB',{minimumFractionDigits:2,maximumFractionDigits:2});
	const canSave = paymentMode && date && Number(amount) > 0;
	...
	return (<>
		...
		{/* Form Card */}
        <div style={{borderRadius:'16px',border:'1px solid #eaecf2',background:'#fff',overflow:'visible',boxShadow:'0 1px 2px rgba(0,0,0,0.03),0 4px 16px rgba(0,0,0,0.04)',padding: isMobile ? '16px' : '24px 28px'}}>
			<form onSubmit={formik.handleSubmit}>
				{/* Supplier */}
				<div style={{marginBottom:'18px'}}>
					<label style={isMobile ? {fontSize:'10px',fontWeight:'700',color:'#111827',letterSpacing:'0.7px',textTransform:'uppercase',marginBottom:'10px',display:'block'} : lblStyle}>Supplier</label>
					{initialSupplierId ? (
						isMobile ? (
							<div style={{display:'flex',alignItems:'center',gap:'12px',background:'#F3F4F6',border:'none',borderRadius:'14px',padding:'0 14px',height:'52px',cursor:'not-allowed'}}>
								<i className="fa fa-truck" style={{fontSize:'18px',color:'#9CA3AF',flexShrink:0}}></i>
								<span style={{fontWeight:'600',fontSize:'15px',color:'#1e293b',flex:1}}>{(supplierOptions.find(o => o.value === currentSupplier) || {}).label || 'Loading...'}</span>
							...
							<div style={{display:'flex',alignItems:'center',gap:'10px',background:'#f1f5f9',border:'1.5px solid #e2e8f0',borderRadius:'10px',padding:'0 16px',height:h,cursor:'not-allowed',opacity:0.85,maxWidth:'350px'}}>
								<i className="fa fa-truck" style={{fontSize:'12px',color:'#94a3b8'}}></i>
								<span style={{fontWeight:'700',fontSize:'14px',color:'#1e293b'}}>{(supplierOptions.find(o => o.value === currentSupplier)||{}).label||'Loading...'}</span>
								<i className="fa fa-lock" style={{fontSize:'11px',color:'#cbd5e1',marginLeft:'auto'}}></i>
							</div>
						)
					) : (
						<div style={{maxWidth: isMobile ? '100%' : '350px'}}>
							<Select styles={selectCtrl} options={supplierOptions} value={supplierOptions.find(o=>o.value===currentSupplier)||null}
								isLoading={suppliersLoading} isClearable isSearchable onChange={handleSupplierChange} classNamePrefix="react-select" placeholder="Select Supplier" menuPortalTarget={document.body} />
						</div>
					)}
				</div>
				...
				<div style={{display:'flex',gap:'12px',flexWrap:'wrap',alignItems:'flex-end'}}>
					{paymentMode === 'on-account'
						? <div style={{flex:1,minWidth:'180px'}}>
							...
							<Select styles={selectCtrl} options={options} isLoading={loading} isClearable isSearchable onChange={handleChange} classNamePrefix="react-select" menuPortalTarget={document.body} />
						</div>
					...
					<div style={{flex:'0 0 160px'}}>
						...
						<OrangeDatePicker value={date} onChange={(val) => { formik.setFieldValue('date', val); dispatch(setDate(val)); }} />
						{formik.touched.date && formik.errors.date && <div style={errStyle}>{formik.errors.date}</div>}
					</div>
					...
					{/* Save Button */}
					<button type="button" disabled={!canSave} onClick={() => dispatch(triggerSave())}
						style={{height:h,padding:'0 28px',borderRadius:'12px',border:'none',
							background:canSave?'rgb(234, 88, 12)':'#e5e7eb',
							color:canSave?'#fff':'#9ca3af',fontSize:'14px',fontWeight:'700',
						...
						<i className="fa fa-check-circle"></i> Save Payment
					</button>
				</div>
				)}
			</form>
...
// list.
function List(props) {
	const {suppliers, totalChecked, invoices,amount,paymentMode,date,note,paymentDoable,onAccount,saveTrigger} = useSelector(state => state.suppliers);
	const currentSupplier = useSelector(state => state.suppliers.currentSupplier);
    ...
    const columns = [
        ...
        { name: 'Invoice No.', selector: row => row.id, sortable: true },
        { name: 'Other Invoice No.', selector: row => (<b>{row.other_invoice_id}</b>), sortable: false },
		{ name: 'Amount', selector: row => row.total_products, sortable: false },
        { name: 'Paid', selector: row => row.total_payments, right: false },
		{ name: 'Balance', selector: row => row.balance_due, right: false },
		{ name: 'Running Balance', selector: row => row.running_balance },
	...
	const loadList = async() => {
		try {
			const response = await axios.post('/payments/supplier_payment/create/unpaid-invoices/'+currentSupplier);
			if (response.data.success === true) {
	...
	const save = async() => {
		...
		if (invoices.length > 0) {
			...
			if (Number(amount) > selectedTotal) {
				notifyError('Amount (' + Number(amount).toFixed(2) + ') exceeds the selected invoices total (' + selectedTotal.toFixed(2) + ')');
				return;
			}
		}
		try {
			...
			setSaving(1)
			const response = await axios.post('/payments/supplier_payment/create/pay/'+currentSupplier, params);
			if (response.data.success === true) {
	...
	useEffect(() => {
		if (saveTrigger > 0 && currentSupplier && Number(amount) > 0 && paymentMode && date) {
			savePayment();
		}
	}, [saveTrigger]);
						...
						<tbody>
							{data.map(row => (
								<tr key={row.id} style={{borderBottom:'1px solid #f3f4f6'}}>
									<td style={{padding:'14px 22px'}}><a href={`/data_entry/purchase_entry/invoice/${row.id}`} style={{textDecoration:'none'}}><span style={{background:'#FFF7ED',border:'1px solid #fed7aa',borderRadius:'6px',padding:'3px 10px',fontWeight:'700',fontSize:'12px',color:'rgb(234, 88, 12)'}}>#{row.id}</span></a></td>
...
export default function SupplierPaymentApp(props) {
	const dispatch = useDispatch();
    const suppliers = useSelector(state => state.suppliers.suppliers);
	...
	<div style={{maxWidth:'1440px',margin:'0 auto'}}>
		<div style={{marginBottom:'20px'}}>
			<CreateForm apiUrl={'/payments/supplier_payment/create/suppliers/list'} initialSupplierId={props.supplierId} currency={props.currency} />
		</div>
...
// ----------------- Mount App -----------------
if (document.getElementById('supplier-payment-app')) {
    const id = "supplier-payment-app";
    const root = createRoot(document.getElementById(id));
    const element = document.getElementById(id);
    const props = Object.assign({}, element.dataset)
    root.render(
		<Provider store={store}>
			<SupplierPaymentApp {...props} />
		</Provider>
    );
}
