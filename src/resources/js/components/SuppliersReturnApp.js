...
// Actions cell extracted to its own top-level component so its hooks
// (useState/useRef) are called at the component's own top level — one
// consistent order per mounted cell. react-data-table-component invokes the
// `cell` render once per row, so calling hooks inline there made the hook count
// vary with row count (pagination/filter/data changes) → React #300.
			...
			{returnModal.show && returnModal.item && (<>
				...
				<div style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',background:'#fff',borderRadius:'16px',width:'440px',maxWidth:'90vw',zIndex:99999,boxShadow:'0 20px 60px rgba(0,0,0,0.2)',overflow:'hidden'}}>
					...
					<div style={{padding:'18px 22px'}}>
						<div style={{display:'flex',...(isMobile ? {flexWrap:'wrap'} : {}),gap:'10px',marginBottom:'16px'}}>
							<div style={{flex:1,...(isMobile ? {minWidth:0} : {}),background:'#f8fafc',borderRadius:'10px',padding:'10px 14px',textAlign:'center'}}><div style={{fontSize:'9px',fontWeight:'700',color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.4px'}}>Invoice</div><div style={{fontSize:'15px',fontWeight:'800',color:'rgb(234, 88, 12)',marginTop:'2px'}}>#{returnModal.item.invoice_id}</div></div>
							<div style={{flex:1,...(isMobile ? {minWidth:0} : {}),background:'#f0fdf4',borderRadius:'10px',padding:'10px 14px',textAlign:'center'}}><div style={{fontSize:'9px',fontWeight:'700',color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.4px'}}>Available</div><div style={{fontSize:'15px',fontWeight:'800',color:'#16a34a',marginTop:'2px'}}>{returnModal.item.available}</div></div>
						...
						<div style={{background:'#fef2f2',borderRadius:'10px',padding:'14px 16px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
							...
							<div style={{display:'flex',alignItems:'center',gap:'6px'}}><span style={{fontSize:'14px',fontWeight:'700',color:'#dc2626'}}>{currency}</span><input type="number" min="0" step="0.01" value={customPrice !== '' ? customPrice : autoPrice.toFixed(2)} onChange={e => setCustomPrice(e.target.value)} style={{...(isMobile ? {flex:1,maxWidth:'140px',minWidth:0} : {width:'100px'}),height:'36px',borderRadius:'8px',border:'1.5px solid #fca5a5',fontSize:'16px',fontWeight:'800',color:'#dc2626',textAlign:'right',padding:'0 8px',outline:'none',background:'#fef2f2'}} onFocus={e=>e.target.style.borderColor='#dc2626'} onBlur={e=>e.target.style.borderColor='#fca5a5'} /></div>
						</div>
					</div>
