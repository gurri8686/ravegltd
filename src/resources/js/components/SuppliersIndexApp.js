...
const slice = createSlice({
    name: 'properties',
    initialState: { refresh: 0, fromDate: '', toDate: '', status: '' },
    reducers: {
...
export default function SuppliersIndexApp(props) {
	...
	const isTablet = false; // iPad/tablets (768-1024) now render the desktop UI (see CLAUDE.md iOS12 notes)
	...
	const columns = [
        {
            name: <span style={hs}>Supplier</span>,
            selector: row => row.name,
            sortable: true,
			cell: row => (
				<div style={{display:'flex',alignItems:'center',gap:'10px'}}>
					<span style={{width:'36px',height:'36px',borderRadius:'50%',background:'rgb(234, 88, 12)',display:'inline-flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:'13px',fontWeight:'700',color:'#fff'}}>{(row.name || '?').charAt(0).toUpperCase()}</span>
					<div style={{lineHeight:'1.3',minWidth:0,flex:1}}>
						<div style={{fontWeight:'700',color:'#1e293b',fontSize:'13px',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{row.name}</div>
						<div style={{fontSize:'11px',color:'#9ca3af',fontWeight:'500',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
							{row.mobile ? <span><i className="fa fa-phone" style={{fontSize:'9px',marginRight:'3px'}}></i>{row.mobile}</span> : null}
							{row.mobile && row.email ? <span style={{margin:'0 5px'}}>·</span> : null}
							{row.email ? <span style={{maxWidth:'160px',overflow:'hidden',textOverflow:'ellipsis',display:'inline-block',verticalAlign:'bottom'}}>{row.email}</span> : null}
							{!row.mobile && !row.email ? <span style={{color:'#d1d5db'}}>No contact info</span> : null}
						</div>
					</div>
				</div>
			),
        ...
        },
		{
            ...
            selector: row => Number(row.total_purchases) || 0,
            sortable: true,
			cell: row => {
				const v = Number(row.total_purchases) || 0;
				return <div style={{width:'100%',textAlign:'right'}}>{v > 0
        ...
        },
        {
            ...
            cell: row => (
				<div style={{display:'flex',gap:'4px',justifyContent:'flex-end'}}>
					{[
						{href:`/payments/supplier_payment/view?supplier=${row.id}`,icon:'fa fa-credit-card',title:'Payments',color:'rgb(234, 88, 12)'},
						{href:`/supplier_return/view?supplier=${row.id}`,icon:'fa fa-undo',title:'Returns',color:'#6366f1'},
						{href:`/supplier_history/view?supplier=${row.id}`,icon:'fa fa-history',title:'History',color:'#0ea5e9'},
						{href:`/management/suppliers/edit/${row.id}/edit`,icon:'fa fa-pencil',title:'Edit',color:'#64748b'},
					].map((btn,i) => (
						<a key={i} href={btn.href} title={btn.title}
							style={{width:'30px',height:'30px',borderRadius:'8px',border:'none',background:'transparent',display:'inline-flex',alignItems:'center',justifyContent:'center',textDecoration:'none',color:'#9ca3af',fontSize:'13px',transition:'all 0.15s',cursor:'pointer'}}
	...
	useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (!cancelled) console.error('Failed to load suppliers', err);
            } finally {
	...
	const filteredData = useMemo(() => {
		...
		return data.filter(row =>
			['name','email','mobile','supplier_id','created_at'].some(field =>
	...
	<>
	<style>{`
		.supp-range-scroll { -webkit-appearance: none; width: 100%; height: 6px; border-radius: 10px; background: #f0f0f0; outline: none; cursor: pointer; }
		.supp-range-scroll::-webkit-slider-thumb { -webkit-appearance: none; width: 50px; height: 6px; border-radius: 10px; background: rgb(234, 88, 12); cursor: pointer; }
		.supp-range-scroll::-moz-range-thumb { width: 50px; height: 6px; border-radius: 10px; background: rgb(234, 88, 12); cursor: pointer; border: none; }
		...
		@media (max-width: 767px) {
			.supp-filter-bar { padding: 0 !important; background: transparent !important; border: none !important; box-shadow: none !important; margin-bottom: 12px !important; }
			.supp-filter-row { flex-direction: column !important; align-items: stretch !important; gap: 10px !important; flex-wrap: nowrap !important; }
			.supp-search-col { min-width: unset !important; flex: unset !important; width: 100% !important; }
			.supp-inline-row { display: flex !important; gap: 8px !important; }
			.supp-status-col { flex: 1 1 0 !important; min-width: 0 !important; }
			.supp-date-col { flex: 1 1 0 !important; min-width: 0 !important; }
			.supp-date-col > button { min-width: unset !important; width: 100% !important; font-size: 12px !important; padding: 0 10px !important; }
		}
	`}</style>
	{/* Filters bar */}
	<div className="supp-filter-bar" style={{background:'#fff',borderRadius:'0',border:'1px solid #eaecf2',borderTop:'none',borderBottom:'none',boxShadow:'none',padding:'14px 20px',marginBottom:'0'}}>
		<div className="supp-filter-row" style={{display:'flex',alignItems:'center',gap:'12px',flexWrap:'wrap'}}>
			{/* Search */}
			{isMobile ? (
			<div className="supp-search-col" style={{display:'flex',alignItems:'center',gap:'9px',background:'#fff',border:'1px solid #ececec',borderRadius:'12px',padding:'0 13px',height:'46px'}}>
				...
				<input placeholder="Search suppliers…" value={searchTerm} onChange={e => { setSearchTerm(e.target.value); setMobilePage(0); }} style={{flex:1,minWidth:0,border:'none',outline:'none',fontSize:'14px',fontFamily:'inherit',background:'transparent'}}/>
			</div>
			) : (
			<div className="supp-search-col" style={{position:'relative',flex:'1',minWidth:'200px'}}>
...
{searchTerm && <button type="button" onClick={() => {setSearchTerm(''); setMobilePage(0)}} style={{position:'absolute',right:'10px',top:'50%',transform:'translateY(-50%)',background:'none',border:'none',cursor:'pointer',padding:'0',lineHeight:1,display:'flex',alignItems:'center'}}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>}
			...
			<div className="supp-inline-row" style={{display:'contents'}}>
				{/* Status filter */}
				<div className="supp-status-col" style={{minWidth:'150px'}}>
					<Select
	...
	{isMobile ? (() => {
		...
		const chip = {display:'inline-flex',alignItems:'center',gap:'4px',fontSize:'11px',fontWeight:'600',color:'#475569',background:'#f8fafc',border:'1px solid #eaecf2',borderRadius:'7px',padding:'4px 9px',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',minWidth:0};
			...
			<div>
				...
				<div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'12px'}}>
					<span style={{display:'inline-flex',alignItems:'center',gap:'8px',fontSize:'14px',fontWeight:'700',color:'rgb(234,88,12)'}}>
						<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgb(234,88,12)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0}}><rect x="1" y="3" width="15" height="13" rx="2"/><path d="M16 8h4l3 3v5h-7z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>{totalItems} suppliers
					</span>
				...
				{isLoading && data.length === 0 ? (
					<SpecTableLoading label="Loading suppliers…" />
				) : totalItems === 0 ? (
					<div style={{background:'#fff',border:'1px solid #eaecf2',borderRadius:'14px',boxShadow:'0 1px 4px rgba(0,0,0,0.05)',padding:'40px 24px',textAlign:'center'}}>
						...
						<div style={{fontSize:'15px',fontWeight:'800',color:'#0f1115'}}>No suppliers found</div>
						<div style={{fontSize:'13px',color:'#6b7280',marginTop:'6px',lineHeight:'1.5',maxWidth:'320px',marginInline:'auto'}}>{(searchTerm || status || fromDate || toDate) ? 'Try changing the search term or filters.' : 'New suppliers appear here as soon as you add them.'}</div>
					...
					<div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
						{cardSlice.map((row,i)=>{
							...
							const total    = Number(row.total_purchases) || 0;
								...
								<div key={row.id||i} style={{position:'relative',background:'#fff',border:'1px solid #eaecf2',borderRadius:'14px',boxShadow:'0 1px 4px rgba(0,0,0,0.05)',padding:'15px 16px 14px',overflow:'hidden'}}>
									...
									<div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'10px'}}>
										<div style={{display:'flex',alignItems:'center',gap:'11px',minWidth:0}}>
											<span style={{width:'42px',height:'42px',borderRadius:'50%',background:'rgb(234, 88, 12)',display:'inline-flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:'16px',fontWeight:'800',color:'#fff'}}>{(row.name||'?').charAt(0).toUpperCase()}</span>
									...
									<div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',marginTop:'14px',flexWrap:'wrap',gap:'10px'}}>
										<div style={{minWidth:0}}>
											<div style={{fontSize:'10px',fontWeight:'700',color:'#94a3b8',letterSpacing:'0.6px',textTransform:'uppercase'}}>Total Purchases</div>
					...
					<div style={{borderRadius:'14px',border:'1px solid #eaecf2',boxShadow:'0 1px 6px rgba(0,0,0,0.05)',background:'#fff',overflow:'hidden'}}>
						<div style={{overflowX:'auto',WebkitOverflowScrolling:'touch'}}>
							<div style={{minWidth:'900px'}}>
								<DataTable columns={columns} data={cardSlice} responsive={false} highlightOnHover customStyles={mergedStyles} persistTableHead noDataComponent={<div style={{padding:'40px',textAlign:'center',color:'#94a3b8',fontSize:'13px'}}>No suppliers found</div>} />
							</div>
						</div>
		...
		const fmt2 = (v) => { const n = Number(v)||0; return n.toLocaleString('en-GB',{minimumFractionDigits:2,maximumFractionDigits:2}); };
		const colGrid = isTablet ? '220px 90px 110px 110px 110px 110px 140px' : '180px 90px 100px 100px 100px 100px 116px';
		const mobileActionBtns = (row) => [
			{href:`/payments/supplier_payment/view?supplier=${row.id}`,icon:'fa fa-credit-card',title:'Payments',color:'rgb(234, 88, 12)',bg:'#FFF5ED'},
			{href:`/supplier_return/view?supplier=${row.id}`,icon:'fa fa-undo',title:'Returns',color:'#6366f1',bg:'#ede9fe'},
			{href:`/supplier_history/view?supplier=${row.id}`,icon:'fa fa-history',title:'History',color:'#0ea5e9',bg:'#e0f2fe'},
			{href:`/management/suppliers/edit/${row.id}/edit`,icon:'fa fa-pencil',title:'Edit',color:'#64748b',bg:'#f1f5f9'},
		];
		return (
		<div style={{background:'#fff',borderRadius:'0 0 16px 16px',border:'1px solid #eaecf2',borderTop:'none',boxShadow:'0 4px 16px rgba(0,0,0,0.04)',overflow:'hidden'}}>
			...
			<div className="supp-scroll-inner" style={{minWidth: filteredData.length > 0 ? (isTablet ? '890px' : '786px') : 'auto', transition:'transform 0.15s ease'}}>
			{/* Column headers */}
			<div style={{display:'grid',gridTemplateColumns:colGrid,padding:'10px 14px',background:'#fff',borderBottom:'1.5px solid #f1f5f9'}}>
				<span style={{fontSize:'10.5px',fontWeight:'700',color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.8px'}}>Supplier</span>
			...
			{pageItems.length === 0 ? (
				isLoading ? (
					<div style={{padding:'40px 16px',textAlign:'center'}}>
						<div style={{display:'inline-flex',alignItems:'center',gap:'8px',padding:'9px 16px',background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:'9999px',color:'rgb(234, 88, 12)',fontSize:'12.5px',fontWeight:'600'}}>
					...
					<div style={{padding:'40px 16px',textAlign:'center'}}>
						<i className="fa fa-truck" style={{fontSize:'32px',color:'#e5e7eb',display:'block',marginBottom:'10px'}}></i>
						<div style={{fontSize:'13px',fontWeight:'600',color:'#9ca3af'}}>No suppliers found</div>
					</div>
				)
			) : pageItems.map((row, i) => {
				const invoices = Number(row.total_invoices) || 0;
	...
	<div style={{borderRadius:'0 0 16px 16px',border:'1px solid #eaecf2',borderTop:'none',boxShadow:'0 4px 16px rgba(0,0,0,0.04)',background:'#fff',overflow:'hidden',position:'relative'}}>
		<DataTable
			...
			progressComponent={<SpecTableLoading label="Loading suppliers…" />}
			noDataComponent={
				<div style={{padding:'48px 24px',textAlign:'center'}}>
					<i className="fa fa-truck" style={{fontSize:'36px',color:'#e5e7eb',display:'block',marginBottom:'12px'}}></i>
					<div style={{fontSize:'14px',fontWeight:'600',color:'#9ca3af'}}>No suppliers found</div>
		...
		{isLoading && data.length > 0 && (
			<div style={{position:'absolute',inset:0,background:'rgba(255,255,255,0.55)',display:'flex',alignItems:'flex-start',justifyContent:'center',pointerEvents:'none',paddingTop:'72px',zIndex:5}}>
				<div style={{display:'inline-flex',alignItems:'center',gap:'10px',padding:'10px 18px',background:'#ffffff',border:'1px solid #fed7aa',borderRadius:'9999px',color:'rgb(234, 88, 12)',fontSize:'13px',fontWeight:'600',boxShadow:'0 4px 12px rgba(15,23,42,0.10)'}}>
...
// ----------------- Mount App -----------------
if (document.getElementById('suppliers-index-app')) {
    const id = "suppliers-index-app";
    const root = createRoot(document.getElementById(id));
    const element = document.getElementById(id);
    const props = Object.assign({}, element.dataset)
    root.render(
		<Provider store={store}>
			<SuppliersIndexApp {...props} />
		</Provider>
    );
}
